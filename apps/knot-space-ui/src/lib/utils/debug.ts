 ...
 */
export function validatePackageData(pkg: Package): { isValid: boolean; issues: string[] } {
  const issues: string[] = [];
  ...
  if (pkg.downloadsCount === undefined || pkg.downloadsCount === null) {
    ...
    const downloadCount = typeof pkg.downloadsCount === 'string' 
  ...
  if (pkg.fileSize === undefined || pkg.fileSize === null) {
    ...
    const fileSize = typeof pkg.fileSize === 'string' 
...
export function debugPackageData(packages: Package[]): void {
  ...
  if (invalidCount === 0) {
    ...
    console.warn(`⚠️ ${invalidCount}/${totalPackages} packages have data issues`);
  }
  
  // Summary stats
  const totalDownloads = packages.reduce((sum, pkg) => {
    const count = typeof pkg.downloadsCount === 'string' 
      ? parseInt(pkg.downloadsCount, 10) 
      : (pkg.downloadsCount || 0);
    return sum + (isNaN(count) ? 0 : count);
  }, 0);
  
  console.log(`📊 Total downloads across all packages: ${totalDownloads.toLocaleString()}`);
...
export function validateApiResponse(response: any, endpoint: string): void {
  ...
  if (response.success === false) {
    console.warn('API returned success: false', response.error || response.message);
  }
