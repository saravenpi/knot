    ...
    async fetchByName(name: string, version?: string) {
      ...
      try {
        ...
        if (version) {
          ...
          pkg = await packagesApi.getByNameAndVersion(
            decodeURIComponent(name),
            version,
          );
        } else {
          // Get latest version by getting all versions and picking the first one
          const versions = await packagesApi.getVersions(
            decodeURIComponent(name),
          );
    ...
    async delete(name: string, version: string) {
      ...
      try {
        ...
        update((state) => ({
          ...
          selectedPackage:
            state.selectedPackage?.name === name &&
            state.selectedPackage?.version === version
              ? null
              : state.selectedPackage,
          loading: false,
        }));
