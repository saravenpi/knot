...
pub async fn run_script_interactive() -> Result<()> {
    ...
    let project = match Project::find_and_load(&current_dir) {
        ...
        Err(_) => {
            println!("❌ No knot.yml found");
            println!("💡 Run from a directory containing knot.yml, app.yml, or package.yml");
            return Ok(());
        }
    };
    ...
    match selection {
        Ok(selected_text) => {
            ...
            let working_dir = if context == "project" && project.is_some() {
                &project.as_ref().unwrap().root
            } else {
                &current_dir
            };
